...
let buttonsRP = document.getElementsByClassName(
...
const targets = [`RedeemRPProduct('free_points_100')`];
  ...
  const checker = Array.from(buttonsRP).find(index => {
    if (
      index
        ...
        .toLocaleLowerCase()
        .includes(t.toLocaleLowerCase()) === true
    ) {
      ...
      index
        .getAttribute("onclick")
        .toLocaleLowerCase()
        .includes(`RedeemRPProduct('free_lott_100')`.toLocaleLowerCase()) ===
        true &&
      parseFloat(rpBalance.replace(/,/g, "")) > 2500
    ) {
      index.removeAttribute("disabled");
      return index.click();
    }
